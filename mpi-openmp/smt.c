...
double check_layer(for_coord_t offset[3], double u[offset[0].count_elem + 2][offset[1].count_elem + 2][offset[2].count_elem + 2], 
                   double ax, double ay, double az, double a_t, double hx, double hy, double hz, double t, int threads) {
...
void next_iter(int n, MPI_Comm comm_cart, MPI_Datatype xy_side, MPI_Datatype xz_side, MPI_Datatype yz_side, 
               ...
               double sqhx, double sqhy, double sqhz, double sqathau, int t) {
...
#pragma omp parallel for collapse(3) num_threads(threads)
    ...
    next_iter(n, comm_cart, xy_side, xz_side, yz_side, offset, u1, u0, hx * hx, hy * hy, hz * hz, sqathau / 2, threads);
    ...
    for (int i = 2; i <= steps; i++) {
        next_iter(n, comm_cart, xy_side, xz_side, yz_side, offset, arr, cur, hx * hx, hy * hy, hz * hz, sqathau, threads);
        tmp = cur;
        cur = arr;
        arr = tmp;
    }
    ...
    if (rank == 0) {
        ...
        printf("Max deviation u%d: %.12lg\n", steps, check_layer(offset, cur, ax, ay, az, a_t, hx, hy, hz, time, threads));
        printf("Time spent: %0.6lg\n\n\n", end_time - start_time);
    }
